...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/users", async (route) => route.fulfill({ json: users }));
  await page.route("**/api/departments", async (route) => route.fulfill({ json: departments }));
  await page.route("**/api/policies", async (route) => route.fulfill({ json: policies }));
  await page.route("**/api/meetings", async (route) => route.fulfill({ json: meetings }));
  await page.route("**/api/decisions", async (route) => route.fulfill({ json: decisions }));
  await page.route("**/api/action-items", async (route) => route.fulfill({ json: actionItems }));
  ...
  await page.route("**/api/tenants", async (route) => route.fulfill({ json: tenants }));
  await page.route("**/api/documents", async (route) => route.fulfill({ json: [{ id: 1, tenant_id: 1, title: "Evidence Pack", filename: "evidence.pdf", content_type: "application/pdf", file_size: 1024, storage_path: "storage/evidence.pdf", linked_entity_type: "policy", linked_entity_id: 1, uploaded_by: 1, created_at: "2026-01-01T00:00:00Z" }] }));
  await page.route("**/api/notifications", async (route) => route.fulfill({ json: [{ id: 1, tenant_id: 1, user_id: 1, title: "Approval due", message: "Review policy", status: "unread", due_date: "2026-06-30", created_at: "2026-01-01T00:00:00Z" }] }));
  await page.route("**/api/calendar-events", async (route) => route.fulfill({ json: [] }));
  await page.route("**/api/workflow-steps", async (route) => route.fulfill({ json: [] }));
  await page.route("**/api/integrations", async (route) => route.fulfill({ json: [{ id: 1, tenant_id: 1, name: "GRC Export", integration_type: "compliance", endpoint_url: null, status: "configured", created_at: "2026-01-01T00:00:00Z" }] }));
  ...
  await page.route("**/api/sso-providers", async (route) => route.fulfill({ json: [{ id: 1, tenant_id: 1, name: "Corporate SSO", provider_type: "saml", metadata_url: "https://idp.example/metadata", status: "enabled", created_at: "2026-01-01T00:00:00Z" }] }));
  await page.route("**/api/compliance-obligations", async (route) => route.fulfill({ json: [{ id: 1, tenant_id: 1, title: "SOX evidence", source: "SOX", owner_id: 2, due_date: "2026-07-01", status: "in_progress", evidence_document_id: 1, description: "Collect evidence", created_at: "2026-01-01T00:00:00Z" }] }));
  await page.route("**/api/risks", async (route) => route.fulfill({ json: [{ id: 1, tenant_id: 1, title: "Evidence gap", category: "audit", severity: "high", status: "open", owner_id: 2, mitigation_plan: "Weekly review", created_at: "2026-01-01T00:00:00Z" }] }));
});
